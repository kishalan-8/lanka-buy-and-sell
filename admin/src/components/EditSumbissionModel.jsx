...
const EditSubmissionModel = ({ submission, setIsOpen, refresh }) => {
  ...
  const [retainedImages, setRetainedImages] = useState([]); // Existing images
  ...
  useEffect(() => {
    if (submission) {
      setForm({
        ...
        condition: submission.condition || "new",
        description: submission.description || "",
        ownerName: submission.ownerName || "",
        ownerContact: submission.ownerContact || "",
      });
      setRetainedImages(submission.images || []);
      setDocuments(
  ...
  const handleFileChange = (e, type) => {
    if (type === "images") setImages([...images, ...Array.from(e.target.files)]);
    else setNewDocs((prev) => ({ ...prev, [type]: Array.from(e.target.files) }));
  };

  const removeExistingImage = (index) => setRetainedImages((prev) => prev.filter((_, i) => i !== index));
  ...
  const renderPreview = (doc) => {
    if (!doc.file && !doc.fileUrl) return null;
    const url = doc.file ? URL.createObjectURL(doc.file) : doc.fileUrl;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Existing images
      retainedImages.forEach((url) => data.append("existingImages", url));
      ...
      Object.keys(newDocs).forEach((type) => {
        newDocs[type].forEach((file) => data.append(type, file));
      });

      await axios.put(`http://localhost:5000/api/submissions/${submission._id}`, data, {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <motion.div className="bg-white rounded-xl max-w-4xl w-full p-6 relative overflow-auto max-h-[90vh]">
        {/* Tabs */}
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {retainedImages.map((url, index)=>(
                <div key={`old-${index}`} className="relative w-20 h-20 rounded-md overflow-hidden border">
                  <img src={url} alt={`bike-old-${index}`} className="w-full h-full object-cover"/>
                  <button type="button" onClick={()=> setRetainedImages(prev => prev.filter((_, i)=> i!==index))} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 text-xs flex items-center justify-center">×</button>
                </div>
              ))}
              {images.map((file, index)=>(
                <div key={`new-${index}`} className="relative w-20 h-20 rounded-md overflow-hidden border">
                  <img src={URL.createObjectURL(file)} alt={`bike-new-${index}`} className="w-full h-full object-cover"/>
                  <button type="button" onClick={()=> setImages(prev => prev.filter((_, i)=> i!==index))} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 text-xs flex items-center justify-center">×</button>
                </div>
              ))}
            </div>
          </div>
...
export default EditSubmissionModel;
