...
const DOCUMENT_TYPES = ["Bike Book", "Revenue License", "Insurance", "Emmision Test", "Transfer Document", "Old Owner ID", "New Owner ID"];

const AddSoldBikeModel = ({ isOpen, onClose, refreshSoldBikes }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post("http://localhost:5000/api/sold", data, {
      ...
      >
        ...
        {activeTab === "info" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <input name="ownerName" placeholder="Old Owner Name" value={form.ownerName} onChange={handleInputChange} className="p-2 border rounded"/>
            <input name="ownerContact" type="number" placeholder="Old Owner Contact" value={form.ownerContact} onChange={handleInputChange} className="p-2 border rounded"/>
            <input name="newOwnerName" placeholder="New Owner Name" value={form.newOwnerName} onChange={handleInputChange} className="p-2 border rounded"/>
            <input name="newOwnerContact" type="number" placeholder="New Owner Contact" value={form.newOwnerContact} onChange={handleInputChange} className="p-2 border rounded"/>
            <input name="soldFor" type="number" placeholder="Sold For" value={form.soldFor} onChange={handleInputChange} className="p-2 border rounded"/>
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <button onClick={handleSubmit} className="px-4 py-2 bg-green-500 text-white rounded" disabled={loading}>
            {loading ? "Saving..." : "Add Sold Bike"}
          </button>
        </div>
      </motion.div>
    </div>
...
export default AddSoldBikeModel;
