...
const BikeCard = ({ bike }) => {
  ...
  // Check if bike is in wishlist on mount
  useEffect(() => {
    const fetchWishlist = async () => {
      ...
      try {
        ...
        const wishlistArray = Array.isArray(res.data)
          ? res.data
          : Array.isArray(res.data.data)
          ? res.data.data
          : [];

        const saved = wishlistArray.some(
          (item) => item.bike?._id === bike._id || item.bikeId === bike._id
        );
        setIsSaved(saved);
      } catch (err) {
  ...
  const handleSaveToggle = async (e) => {
    ...
    try {
      if (!isSaved) {
        ...
        await axios.delete(`http://localhost:5000/api/wishlist/${bike._id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setIsSaved(false);
      }
